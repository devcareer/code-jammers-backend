...
export default class UserController {
  static async createUser(req, res) {
    try {
      ...
      if (emailExist) {
        return res.status(409).json({ status: 403, error: "Email already used by another user." });
      }
      const usernameExist = await User.usernameExist(username);
      if (usernameExist) {
        return res.status(409).json({ status: 403, error: `Sorry, ${username} is not available. Please pick another username` });
      }
  ...
  static async loginUser(req, res) {
    try {
      ...
      if (!user) {
        return res.status(409).json({ status: 404, error: "Email does not exist." });
      }
      const validpass = await bcrypt.compare(password, user.password);
      if (!validpass) {
        return res.status(409).json({ status: 404, error: "Password is not correct!." });
      }
